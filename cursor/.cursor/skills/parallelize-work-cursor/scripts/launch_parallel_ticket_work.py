#!/usr/bin/env python3
"""Launch one detached interactive Cursor agent session per task in a wt-managed worktree."""
...
def launch_task(
    *,
    ...
    shell_command = (
        ...
        f"{shlex.quote(worker_bin)} "
        "--force "
        '"$PROMPT"; '
        f"printf '\\n[parallelize-work-cursor] Agent session exited for {task.issue_id}. "
        f"Log: {str(log_path)}\\n'; "
        "exec zsh -l"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch one wt worktree and detached interactive Cursor agent session per task from a JSON plan."
    )
    parser.add_argument("plan", help="Path to the JSON execution plan.")
    parser.add_argument(
        "--run-root",
        help="Directory for generated prompts and logs. Defaults to ~/.cursor/tmp/parallelize-work-cursor/<timestamp>/",
    )
    parser.add_argument(
        "--worker-bin",
        default="agent",
        help="Worker executable to use. Defaults to 'agent'.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    run_root = (
        ...
        if args.run_root
        else Path.home() / ".cursor" / "tmp" / "parallelize-work-cursor" / timestamp
    )
