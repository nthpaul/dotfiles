---
name: parallelize-work-cursor
description: Orchestrate multiple independent implementation tasks in parallel by creating one `wt` worktree and one detached Cursor agent session per task. Use when an implementation document, task breakdown, or Linear project link explicitly identifies which Linear issues can be worked on concurrently.
---

# Parallelize Work (Cursor Agent)

Use this skill to fan out explicitly independent Linear implementation tasks into separate worktrees and detached Cursor agent sessions.

Treat this as an orchestration skill, not an implementation skill. Each child agent should own exactly one Linear issue and should use the `linear-ticket-work` workflow to do the actual work.
...
6. Run `~/.cursor/skills/parallelize-work-cursor/scripts/launch_parallel_ticket_work.py <plan.json>`.
...
3. Start a Cursor agent (`agent` CLI) in that worktree with `--force` to auto-approve tool use.
4. The agent runs interactively inside the detached tmux session so the user can attach later and continue prompting it directly.
5. Save prompt files and pane logs under `~/.cursor/tmp/parallelize-work-cursor/<timestamp>/`.
...
- One task maps to one Linear issue, one worktree, and one child agent run.
...
- Prefer interactive child sessions over one-shot runs so the user can attach and steer any child agent directly.
