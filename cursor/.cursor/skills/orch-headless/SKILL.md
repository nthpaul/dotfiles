...
description: >-
  ...
  panes. Read the orch skill next. Do not implement — spawn grok/cursor with
  --mode headless and wait on result.json.
---
...
3. Every spawn is `--mode headless` unless they ask for a live pane on that one job.
4. Grok or Cursor as the orch skill routes. No tmux. No `orch tell`. Wait on `orch result` / `orch logs`, then `orch kill`.
