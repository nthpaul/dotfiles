...
name: system-atlas
description: >
  Build and maintain an explorable, progressively-disclosed isometric atlas of a
  system's architecture — an interactive HTML map (hover to read, click to pin,
  go inside for steps, moving data packets you can inspect, chapters that reveal
  a few structures at a time) plus a generated text twin (SYSTEM.md) and question
  tracking by ID, all from one data file. Use whenever someone wants to discuss,
  design, review, or explain an architecture visually: "make an atlas", "map the
  system", "make the architecture explorable", "visualize the codebase/agent/pipeline
  so we can talk about it", "a diagram I can click around", "walk me through how it
  fits together", or when an architecture discussion is producing open questions
  that need tracking across feedback rounds. Also use it to update an existing
  atlas after decisions change. Use when the user runs /system-atlas.
license: MIT
compatibility: Requires Node.js to run the atlas build script
argument-hint: "[system name or existing atlas path]"
metadata:
  origin: https://github.com/inkboard/system-atlas
  short-description: Interactive isometric architecture atlas
---
...
`assets/` and `references/` sit next to this file. Copy starter files from that directory — typically `$GROK_HOME/skills/system-atlas` (default `~/.grok/skills/system-atlas`) or `~/.cursor/skills/system-atlas`. Those two paths may be the same directory via symlink. If neither exists, search for `system-atlas/SKILL.md` under the current repo's `.grok/skills`, `.cursor/skills`, and `.agents/skills`. Do not invent a renderer.
...
- Serve the parent folder with a static server (`npx serve`, `python3 -m http.server`) and hand over the local URL. This is the default on Grok and Cursor.
- If the current harness can publish a hosted HTML artifact, do that and keep the URL stable across rebuilds.
- Or commit the file and let the repo's pages host serve it.

Put the published URL in `META.artifactUrl` so `SYSTEM.md` links to it. One URL, republished after every data change, never a second copy.
